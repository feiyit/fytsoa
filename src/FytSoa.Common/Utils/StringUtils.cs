...
public static class StringUtils
{
    ...
    /// <returns></returns>
    public static List<string> StrToListString(this string str,char c=',')
    {
        ...
        var slist = str.Split(new[] { c }, StringSplitOptions.RemoveEmptyEntries);
        list.AddRange(slist);
        return list;
    }
    #endregion
    ...
    /// <param name="s"></param>
    /// <returns></returns>
    public static string ConvertToPinYinFirst(this string s)
    {
        var first = s.First();
        var pinyin = new ChineseChar(first);
        return pinyin.Pinyins[0].First().ToString().ToUpper();
    }
    ...
    /// <param name="s"></param>
    /// <returns></returns>
    public static string ConvertToPinYin(this string s)
    {
        StringBuilder sb = new StringBuilder();
        foreach (var item in s)
        {
            var chineseChar = new ChineseChar(item);
            sb.Append(chineseChar.Pinyins[0].AsSpan(0, chineseChar.Pinyins[0].Length - 1));
        }
        return sb.ToString();
    }
    ...
    /// <returns></returns>
    public static string TaskStatus(this int status)
    {
    ...
    /// <param name="status"></param>
    /// <returns></returns>
    public static string ContractStatus(this int status)
    {
        var str = status switch
        {
            1 => "未审核",
    ...
    #region 编号 OD20230101-001

    /// <summary>
    /// 根据数量设计编号
    /// </summary>
    /// <param name="tag">编号标签</param>
    /// <param name="count">当前数量</param>
    /// <param name="format">格式</param>
    /// <returns></returns>
    public static string ResNumber(this string tag,int count,string format="yyyyMMdd")
    {
        ...
        return tag+DateTime.Now.ToString("yyyyMM")+"-"+number;
    }
    #endregion
}