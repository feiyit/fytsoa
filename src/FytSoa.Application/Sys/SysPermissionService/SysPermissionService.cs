...
public class SysPermissionService : IApplicationService
{
    ...
    public async Task<bool> AddAsync(SysAuthorityParam model)
    {
        ...
        var roleModel = await _roleRepository.GetFirstAsync(m => m.Id == model.RoleId);
        if (roleModel.Workbench!=model.Workbench)
        {
            await _roleRepository.UpdateAsync(m =>new SysRole() {Workbench = model.Workbench },m=>m.Id==model.RoleId);
        }
