...
public class SysCityService : IApplicationService
{
    private readonly SugarRepository<SysCity> _thisRepository;
    public SysCityService(SugarRepository<SysCity> thisRepository)
    {
    ...
    /// <returns></returns>
    public async Task<PageResult<SysCityDto>> GetPagesAsync(PageParam param)
    {
        var query = await _thisRepository.AsQueryable()
            .WhereIF(!string.IsNullOrEmpty(param.Key),m=>m.Name.Contains(param.Key))
            .ToPageAsync(param.Page, param.Limit);
        return query.Adapt<PageResult<SysCityDto>>();
    }
    ...
    /// </summary>
    /// <returns></returns>
    public async Task<List<SysCityDto>> GetListAsync(WhereParam param)
    {
        var list = await _thisRepository.AsQueryable()
            .WhereIF(!string.IsNullOrEmpty(param.Key),m=>m.Name.Contains(param.Key))
            .OrderBy(m=>m.Id,OrderByType.Desc).ToListAsync();
        return list.Adapt<List<SysCityDto>>();
    }
    ...
    /// <returns></returns>
    [HttpGet("{id}")]
    public async Task<SysCityDto> GetAsync(long id)
    {
        var model = await _thisRepository.GetByIdAsync(id);
        return model.Adapt<SysCityDto>();
    }
    ...
    /// <returns></returns>
    public async Task<bool> AddAsync(SysCityDto model)
    {
        ...
        return await _thisRepository.InsertAsync(model.Adapt<SysCity>());
    }
    ...
    /// <param name="model"></param>
    /// <returns></returns>
    public async Task<bool> ModifyAsync(SysCityDto model)
    {
        if (model.ParentIdList.All(m => m != "0"))
        {
            model.ParentId = long.Parse(model.ParentIdList.Last());
        ...
        return await _thisRepository.UpdateAsync(model.Adapt<SysCity>());
    }
