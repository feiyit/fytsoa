...
export default {
	...
	methods: {
		...
		selectMyfilter(item) {
			...
			this.$emit("filterChange", this.recParam(item.filterObj));
			this.drawer = false;
		},
		//立即过滤
		ok() {
			...
			this.$emit("filterChange", this.recParam(this.filterObj));
			this.drawer = false;
		},
		recParam(sourceObj) {
			let filterJson = [];
			for (var obj in sourceObj) {
				filterJson.push({
					FieldName: obj,
					ConditionalType: this.filterObj[obj].split("|")[1],
					FieldValue: this.filterObj[obj].split("|")[0],
				});
			}
			return filterJson;
		},
