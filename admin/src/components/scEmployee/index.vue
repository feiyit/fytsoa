<template>
	<div class="sel-user" v-if="!hideInput">
		<div class="sel-input">
			<div v-if="multiple" class="sel-multple">
				<el-tag
			...
			<div class="sel-value" v-if="!multiple && resUser.length != 0">
				{{ resUser[0].name }}
			</div>
...
export default {
	...
	methods: {
		...
		async getGroup() {
			...
			const res = await this.$API.hrorganization.list.get();
		...
		async initUser() {
			const that = this;
			const res = await this.$API.hremployee.page.get(this.param);
			if (res.code == 200) {
				this.user = res.data;
		...
		userChange(e, m) {
			...
			this.$emit("update:employee", this.resUser);
		},
		delResUser(m) {
