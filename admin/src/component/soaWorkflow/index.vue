...
const props = withDefaults(
  defineProps<{
    ...
    /** 画布缩放比例，默认为 1（仅用于展示时放大/缩小整体流程） */
    scale?: number;
    /** 是否只读展示模式：true 时隐藏所有“快捷增加/编辑”入口 */
    readonly?: boolean;
  }>(),
  {
    scale: 1,
...
const useSelectRef = ref<InstanceType<typeof UseSelect> | null>(null);

// 画布缩放样式（允许外部通过 scale 属性控制整体流程图的放大/缩小）
const boxScaleStyle = computed(() => {
  const s = Number.isFinite(props.scale) && props.scale! > 0 ? props.scale! : 1;
  return {
    transform: `scale(${s})`,
    transformOrigin: "0 0",
  };
...
/* 节点状态高亮：已完成 / 当前 / 未开始 */
.node-wrap-box.node-status-done,
.auto-judge.node-status-done {
  opacity: 0.55;
}

.node-wrap-box.node-status-current,
.auto-judge.node-status-current {
  border: 1px solid #22c55e;
  box-shadow: 0 0 6px 0 rgba(34, 197, 94, 0.35);
}
