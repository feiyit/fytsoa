		...
		<el-header>
			<div class="left-panel">
				...
				<el-button
					...
					v-auth="'exammaterialcategory:delete'"
					@click="batch_del"
				/>
			</div>
...
export default {
	...
	data() {
		return {
			apiObj: this.$API.exammaterialcategory.list,
			list: [],
			param: {
				key: "",
			},
			...
			column: [
				...
				{ prop: "enName", label: "英文名称", width: 100 },
	...
	methods: {
		...
		//删除
		async table_del(row) {
			var res = await this.$API.exammaterialcategory.delete.delete([
				row.id,
			]);
			if (res.code == 200) {
				this.$refs.table.refresh();
		...
		batch_del() {
			this.$confirm(
				...
				.then(async () => {
					...
					var res =
						await this.$API.exammaterialcategory.delete.delete(ids);
					if (res.code == 200) {
						this.$refs.table.refresh();
						loading.close();
						this.$message.success("删除成功");
