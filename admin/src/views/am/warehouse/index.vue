      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'locationId'">
            {{ record.locationObj?.name }}
          </template>
          <template v-if="column.key === 'managerId'">
            {{ record.managerObj?.fullName }}
          </template>
...
import { fetchAmWarehousePage, deleteAmWarehouse } from "@/api/am/warehouse";
...
const columns = [
  ...
  { title: "地点", dataIndex: "locationId", key: "locationId", width: 120 },
  { title: "管理员", dataIndex: "managerId", key: "managerId", width: 120 },
...
const handleBatchDelete = async () => {
  ...
  await deleteAmWarehouse(selectedRows.value.map((x: any) => x.id));
  ElMessage.success("删除成功");
  handleSearch();
};
</script>
