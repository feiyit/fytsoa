  ...
  <el-container class="h-full">
    ...
    <el-main class="bg-card border-border mt-2 rounded-[.5vw]">
      <soaTable
        ref="tableRef"
        :columns="columns"
        :apiObj="fetchAmWarehouseBinPage"
        row-key="id"
        row-serial-number
...
import {
  fetchAmWarehouseBinPage,
  deleteAmWarehouseBin,
...
const columns = [
  { title: "仓库", dataIndex: "warehouseId", key: "warehouseId", width: 200 },
...
const handleBatchDelete = async () => {
  ...
  await deleteAmWarehouseBin(selectedRows.value.map((x: any) => x.id));
  ElMessage.success("删除成功");
  handleSearch();
};
</script>
