...
export default {
	...
	data() {
		return {
			apiObj: this.$API.sysorganize.list,
			list: [],
			param: {
				key: "",
			},
	...
	methods: {
		...
		//删除
		async table_del(row) {
			var res = await this.$API.sysorganize.delete.delete([row.id]);
			if (res.code == 200) {
				this.$refs.table.refresh();
		...
		batch_del() {
			this.$confirm(
				...
				.then(async () => {
					...
					var res = await this.$API.sysorganize.delete.delete(ids);
					if (res.code == 200) {
						this.$refs.table.refresh();
						loading.close();
						this.$message.success("删除成功");
