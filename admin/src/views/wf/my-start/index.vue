...
const query = reactive<MyStartQueryState>({
  tenantId: 0,
...
const parsedFormConfig = computed(() => {
  ...
  try {
    ...
    console.warn("[wf/my-start] 解析表单配置失败");
    return null;
  }
});
...
const openDetail = async (row: WorkflowInstanceDto) => {
  if (!row.id || query.tenantId == null) return;
  detailVisible.value = true;
  detailLoading.value = true;
  try {
    const tenantId = query.tenantId ?? 0;
    const [inst, instHis, taskHis] = await Promise.all([
      getWorkflowInstance(row.id),
      getWorkflowInstanceHistory({
        tenantId,
        instanceId: row.id,
      }),
    ...
    if (inst?.definitionId) {
      ...
      if (model?.modelJson) {
        try {
          ...
          workflowModel.value = null;
          console.warn("[wf/my-start] 解析流程模型 JSON 失败");
        }
      }
    ...
    if (inst?.definitionKey && inst.businessKey) {
      try {
        ...
        if (biz) {
          if (typeof biz.formDataJson === "string" && biz.formDataJson) {
            try {
              ...
              console.warn("[wf/my-start] 解析 formDataJson 失败", e);
            }
          }
          if (!payload && biz.formData) {
            if (typeof biz.formData === "string") {
              try {
                ...
                console.warn("[wf/my-start] 解析 formData 失败", e);
              }
        ...
        console.warn("[wf/my-start] 加载业务表单数据失败", err);
        formModel.value = {};
      }
    }

    // 构建简单的流程预览（不区分 current，仅标记已完成/未完成）
...
<template>
  <el-container class="h-full">
    ...
    >
      <div class="text-base font-medium">我发起的流程</div>
      <el-button type="primary" @click="loadMyStarted">刷新</el-button>
    </el-header>
    ...
    <el-main class="bg-card border-border mt-2 rounded-[.5vw]">
      <soaTable
        ref="tableRef"
        :columns="columns"
        :apiObj="getMyStartedWorkflowInstances"
